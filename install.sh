...
# ══ Download menu ══
if wget -q https://raw.githubusercontent.com/EkromSSH/UDP-HYSTERIA/main/scripts/menu.py -O /opt/hysteria/menu.py 2>/dev/null; then
  ...
  echo "   wget -q https://raw.githubusercontent.com/EkromSSH/UDP-HYSTERIA/main/scripts/menu.py -O /opt/hysteria/menu.py"
  echo "   printf '#!/bin/bash\\npython3 /opt/hysteria/menu.py\\n' > /usr/local/bin/showon && chmod +x /usr/local/bin/showon"
fi

# ══ Fresh update: download latest scripts from GitHub ══
echo -e "\n\033[1;34m==>\033[0m Updating to latest scripts..."
timeout 30 curl -sL --max-time 25 https://raw.githubusercontent.com/EkromSSH/UDP-HYSTERIA/main/web/index.html -o /home/vps/public_html/server/index.html 2>/dev/null
timeout 30 curl -sL --max-time 25 https://raw.githubusercontent.com/EkromSSH/UDP-HYSTERIA/main/scripts/online-check.sh -o /usr/local/bin/online-check.sh 2>/dev/null
timeout 30 curl -sL --max-time 25 https://raw.githubusercontent.com/EkromSSH/UDP-HYSTERIA/main/scripts/sysinfo.sh -o /usr/local/bin/sysinfo.sh 2>/dev/null
timeout 30 curl -sL --max-time 25 https://raw.githubusercontent.com/EkromSSH/UDP-HYSTERIA/main/scripts/vnstat-traffic.sh -o /usr/local/bin/vnstat-traffic.sh 2>/dev/null
timeout 30 curl -sL --max-time 25 https://raw.githubusercontent.com/EkromSSH/UDP-HYSTERIA/main/scripts/menu.py -o /opt/hysteria/menu.py 2>/dev/null
